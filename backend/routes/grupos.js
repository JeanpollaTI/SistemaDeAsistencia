...
// Funci贸n auxiliar para obtener la siguiente matr铆cula formateada (0001, 0002...)
...
router.post("/", authMiddleware, isAdmin, schoolMiddleware, async (req, res) => {
    try {
        ...
        const alumnosProcesados = await Promise.all((alumnos || []).map(async alumno => {
            const data = { ...alumno };
            // Si es un alumno nuevo o no tiene matr铆cula, le asignamos una
            if (!data.matricula) {
...
router.put("/:id", authMiddleware, isAdmin, async (req, res) => {
    try {
        ...
        if (alumnos) {
            const alumnosProcesados = await Promise.all(alumnos.map(async alumno => {
                const data = { ...alumno };
                // Asignar matr铆cula a alumnos nuevos que no la tengan
                if (!data.matricula) {
                    data.matricula = await getNextMatricula();
                }
                if (data._id && String(data._id).startsWith('new-')) {
...
router.delete("/:id", authMiddleware, isAdmin, schoolMiddleware, async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
            return res.status(400).json({ error: "ID de grupo no v谩lido." });
        }
...
router.get("/mis-grupos", authMiddleware, async (req, res) => {
    try {
        ...
        if (!profesorId) {
            return res.status(401).json({ error: "ID de profesor no v谩lido o faltante en el token." });
        }
...
router.get("/:grupoId/calificaciones-admin", authMiddleware, isAdmin, async (req, res) => {
    try {
        ...
        // Validaci贸n de ID para prevenir el error de Mongoose
        if (!mongoose.Types.ObjectId.isValid(grupoId)) {
            return res.status(400).json({ msg: "ID de grupo no v谩lido." });
        }
        ...
        // 1. Obtener TODOS los registros de calificaci贸n asociados a este grupo
        const registros = await Calificacion.find({ grupo: grupoId });

        const calificacionesAdmin = {};
        const { alumnos } = grupo;

        // Usamos las asignaturas del grupo para saber qu茅 materias esperar
        // Combinamos las asignadas con las que est谩n en el orden guardado (para no perder ninguna)
        ...
        // 3. Iterar sobre cada registro de calificaci贸n (cada materia)
        registros.forEach(registro => {
            ...
            alumnos.forEach(alumno => {
                ...
                const promediosBimestrales = [1, 2, 3].map(bimestre => {
                    ...
                    // Si no hay criterios definidos, se considera que no hay calificaci贸n
                    if (criteriosActivos.length === 0) return null;

                    // Recuperar configuraci贸n de tareas visibles (numTareas)
                    ...
                    let pesoTotalAplicable = 0; // 馃専 FIX: Track total weight of active criteria
                    ...
                    // Solo devolvemos la calificaci贸n si el total de criterios definidos suma 100 (sanity check), 
                    // PERO el c谩lculo se basa en el peso aplicable actual.
                    ...
                    // Solo se reporta la calificaci贸n si es mayor a 0 (para evitar 0s falsos)
...
router.get("/global-search", authMiddleware, schoolMiddleware, async (req, res) => {
    try {
        ...
        // --- 馃専 Mejorar B煤squeda: Normalizaci贸n y Sin贸nimos ---
        const normalize = (str) => str.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, "");
        
        // Mapeo de t茅rminos comunes
        const synonyms = {
        ...
        // Dividir en t茅rminos originales para b煤squeda por partes
        ...
        grupos.forEach(grupo => {
            ...
            // Unir t茅rminos: si todos los t茅rminos de b煤squeda coinciden entre (Nombre Alumno + Nombre Grupo)
            const grupoMatchScore = terms.filter(t => t.test(grupo.nombre)).length;
            // O si la consulta normalizada est谩 contenida en el nombre del grupo normalizado
            ...
            grupo.alumnos.forEach(alumno => {
                ...
                // CRITERIO DE MATCH:
                // 1. Los t茅rminos divididos cubren el nombre+grupo
                ...
                if (
                    ...
                    results.push({
                        ...
                        grupo: grupo.nombre,
                        grupoId: grupo._id,
                        asignatura // Incluimos la asignatura para navegaci贸n directa
                    });
                }
            });
        });
...
router.get("/alumno/:alumnoId/ficha", authMiddleware, schoolMiddleware, async (req, res) => {
    try {
        ...
        if (!grupo) {
            return res.status(404).json({ error: "Alumno no encontrado o no pertenece a su instituci贸n." });
        }
        ...
        res.json({
            alumno: {
                ...
                grupoId: grupo._id,
                escuela: school?.name,
                userAsignatura // Para navegaci贸n r谩pida
            },
            calificaciones,
        ...
        res.status(500).json({ error: "Error al obtener la informaci贸n del alumno." });
    }
});



// [GET] /grupos/:id/analytics - Estad韘ticas de rendimiento del grupo
router.get("/:id/analytics", authMiddleware, isAdmin, schoolMiddleware, async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(id)) {
            return res.status(400).json({ error: "ID de grupo no v醠ido." });
        }
        ...
        // --- 2. AGREGAR CALIFICACIONES (DESEMPE袿) ---
        ...
        res.status(500).json({ error: "Error al calcular anal韙icas." });
    }
});
