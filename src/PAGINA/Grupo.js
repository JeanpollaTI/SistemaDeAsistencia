...
function Grupo({ user }) {
  ...
  // --- HANDLERS ---
  const openModal = async (tipo, data = null) => {
    if (tipo === 'gestionarGrupo') {
      ...
      try {
        // 4. CÓDIGO ACTUALIZADO: Usando apiClient
        const res = await apiClient.get(`/asistencia?grupoId=${data._id}&asignatura=${miAsignacion.asignatura}`, getAxiosConfig());
  ...
  const guardarAsistencia = async () => {
    ...
    try {
      // 9. CÓDIGO ACTUALIZADO: Usando apiClient
    ...
    try {
        ...
        alumnosOrdenados.forEach(alumno => {
            ...
            for (let b = 1; b <= NUM_BIMESTRES; b++) {
                // Aquí usamos la función calcularTotales
                const totales = memoCalcularTotales(alumno._id, b, asistenciaData, diasData); 
  ...
  if (loading && user) return <div className="grupo-componente"><p>Cargando...</p></div>;
              ...
              {nuevoGrupo.alumnos.map(a => (
                <li key={a._id}>
                  ...
                  <div className="alumno-actions">
                    <button className="btn-edit-alumno" onClick={() => openModal('editarAlumno', a)}><FaPencilAlt /></button>
          ...
          <div className="admin-view">
            <header className="main-header">
              ...
              <div className="header-actions">
                <button className="btn btn-primary" onClick={() => openModal('gestionarGrupo')}>Crear Grupo</button>
                <button className="btn btn-secondary" onClick={() => openModal('importar')}>Importar Alumnos XLS</button>
              </div>
            </header>
                ...
                {grupos.map(grupo => (
                  <tr key={grupo._id}>
                    ...
                    <td className="acciones-cell">
                      <button className="btn btn-warning" onClick={() => openModal('gestionarGrupo', grupo)}>Editar</button>
                      <button className="btn btn-secondary" onClick={() => openModal('asignar', grupo)}>Asignar</button>
